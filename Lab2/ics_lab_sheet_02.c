...
int main(int argc,char** argv)
{
  ...
  struct sockaddr_ll ll; // network layer 2 structure (Link layer)
  unsigned char dst[6];
  ...
  {
    /// iface is eth1
    /// dest will be Alice and Bob
    /// proto is 0x0806 -> aRP
    /// the pay load should change depending on to whome we are sending the packet, 
    /// all the difference should be the target Ip, and MAC I think -> we need to look for this. 
    printf(
      "usage: ./RAW_Packet.c <iface> <destination> <payloadproto> <payloadfile>\n"
    );
    return EXIT_FAILURE;
  }
